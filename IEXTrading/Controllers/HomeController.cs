    ...
    public class HomeController : Controller
    {
        ...
        ****/
        ...
        public IActionResult TopPicks()
        {
            ...
            stocks = dbContext.Stocksdb
                .Where(s => s.year1ChangePercent < 0.0 && s.day5ChangePercent > 0.05 && s.marketcap >= 300000000.0)
                .OrderBy(s => s.year1ChangePercent)
                .Take(5).ToList();

            return View(stocks);
        }

        public IActionResult TopPicksValue()
        {

            List<Stock> stocks = new List<Stock>();

            stocks = dbContext.Stocksdb
                .Where(s => s.marketcap >= 300000000.0)
                .OrderBy(s => s.EVtoEBITDA)
                .Take(5).ToList();

            return View(stocks);
        }

        public IActionResult PopulateStocks()
        {
            IEXHandler webHandler = new IEXHandler();
            List<Company> companies = webHandler.GetSymbols();
            companies = companies
                .Where(s => s.type == "cs").ToList();
            //companies = companies.GetRange(0, 200);

            List<Stock> stocks = new List<Stock>();

            dbContext.Stocksdb.RemoveRange(dbContext.Stocksdb);
            ...
            foreach (Company company in companies)
            {
                if (company.symbol != null)
                {
                    ...
                    Stock newStock = webHandler2.GetStocks(company.symbol);
                    dbContext.Stocksdb.Add(newStock);
                    dbContext.SaveChanges();
                }
            }
            dbContext.SaveChanges();
        ...
        ****/
        ...
        public IActionResult Reflections()
        {
            return View();
        }
