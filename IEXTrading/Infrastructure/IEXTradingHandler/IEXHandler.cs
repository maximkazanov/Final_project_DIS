    ...
    public class IEXHandler
    {
        ...
        ****/
        ...
        public Stock GetStocks(string symbol)
        {
            ...
            if (!companyList.Equals(""))
            {
                companies = JsonConvert.DeserializeObject<Stock>(companyList, new JsonSerializerSettings { NullValueHandling = NullValueHandling.Ignore });
            }
            if (companies.marketcap > 0 && companies.debt > 0 && companies.EBITDA > 0)
            {
                companies.EVtoEBITDA = (companies.marketcap + companies.debt - companies.cash) / companies.EBITDA;
            }
            else
                companies.EVtoEBITDA = 1000000000;
